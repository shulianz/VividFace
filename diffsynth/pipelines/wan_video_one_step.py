...
class WanVideoOneStepPipeline(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False):
        ...
        pipe = WanVideoOneStepPipeline(device=device, torch_dtype=torch_dtype)
    ...
    def __call__(
        ...
        timestep = torch.tensor([400.0]).to(dtype=self.torch_dtype, device=self.device)
    

        noise_pred_posi = model_fn_wan_video(
            self.dit, motion_controller=self.motion_controller, vace=self.vace,
            x= LR, timestep=timestep,
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # print(f"x.shape:{x.shape}, f:{f}, h:{h}, w:{w}")
    # x.shape:torch.Size([1, 259200, 1536]), f:24, h:120, w:90
    # assert False
